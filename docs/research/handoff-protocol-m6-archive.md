# Handoff Protocol (M6-era archive)

> **已废止 / SUPERSEDED** — 本文描述的是 M6a–M6d 时代的 fullstack spell
> 协议（`api.spec`、`frontend.done`、`review.completed` 等手工 key 约定，
> 以及 critic/fixer/architect 等已删除的 spell 拓扑）。现行协议见
> [`../handoff-protocol.md`](../handoff-protocol.md)（typed produces/consumes
> + minted key + WakeCoordinator）。保留此文仅作历史决策参考。
